...
- **Chacun relit ses propres créneaux dans l'encart**, dans un champ **pleine largeur** `🗓️ Disponibilités`, **groupé par plage et non par personne**. C'est le **seul endroit possible** — un menu déroulant Discord appartient au message, pas au lecteur, donc il ne peut pas être pré-coché différemment pour chaque personne. Sans ça on coche et on ne sait plus ensuite ce qu'on a mis.

  > 🗓️ **Disponibilités**
  > **08–12** · Abarrach, Lorhelyne, Desta
  > **08–16** · Bahlor, Frya
  > **12–16** · Neuroch, Quatrequart
  > *2 inscrits n'ont pas précisé — comptés sur tous les créneaux.*

  ⚠ **Première version rejetée : la plage collée à chaque nom dans les colonnes de postes.** Ces colonnes sont `inline` donc larges d'un tiers d'encart — « Abarrach · 08–10, 14–16 » y tenait sur deux lignes et l'encart devenait un mur en escalier. **On ne peut pas élargir un encart** (la largeur appartient au client Discord) ; le seul levier est `inline`, qui décide colonne étroite ou pleine largeur. Groupé par plage dans un champ pleine largeur, une sortie de **dix personnes tient en quatre lignes** — et ça se lit comme un planning. Le tri se fait sur l'**index du premier bloc**, pas sur le libellé : alphabétiquement, un rally de nuit placerait « 00–06 » avant « 22–00 ». Les blocs consécutifs sont **fusionnés en plages** (`creneaux_resume()`) parce que les gens pensent en tranches continues (« je suis là le matin »), pas en blocs de deux heures : `[0,1]` → `08–12`, `[0,1,2,3]` → `08–16`, `[0,3]` → `08–10, 14–16`. Un index hors plage (durée raccourcie depuis) est ignoré silencieusement. Rien d'affiché quand rien n'est déclaré — l'absence vaut « disponible partout », inutile de l'écrire sous chaque nom.
