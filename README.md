...
- **Dernier Rempart — carte de démarrage et scintillement (2026-09-19, NON DÉPLOYÉE)** : le panneau latéral détaillait les quatre menaces avec leurs valeurs en points et trois paragraphes de règles — **trop précis, et ce n'est pas ce qu'on voit dans un jeu**. Il est ramené au seul **classement de guilde**, comme l'encart « Cette semaine » des cinq autres jeux. Les règles passent dans une **carte de démarrage** à l'intérieur de la scène, sur le modèle d'Orni Flap / Spice Runner / Muad'Dib Rescue : titre, une phrase de consigne, les quatre silhouettes de menaces avec leur seul nom (aucun chiffre), le bouton, et une ligne d'aide portant le record personnel. Les icônes sont les **mêmes canvas** qu'avant (`[data-munition]`, rendues par `RempartView.munition()`), simplement déplacées — donc aucune divergence possible entre la légende et le moteur de rendu. Grille à 4 colonnes et **pas** de `flex-wrap` : en flex la rangée se réorganisait en 3+1 dès que le panneau se resserrait. La ligne d'aide n'est plus masquée sous 550 px puisqu'elle porte maintenant le record. Les deux paragraphes de règles sous la scène sont remplacés par la seule ligne de commandes, comme ailleurs dans le hub.
  - **Grains de sable scintillants** : 190 particules au lieu de 70, en deux populations — le sable ordinaire, et **26 % d'« éclats »** pris dans la lumière rasante. Chaque grain a sa **phase et sa fréquence propres** : le scintillement est un battement individuel, jamais un clignotement global (sinon tout l'écran pulse ensemble et ça se lit comme un défaut d'affichage). Un facteur `rasance` fait accrocher davantage les grains proches de l'horizon, là où le soleil couchant est bas. Les éclats sont dessinés en **deux disques** (halo à bas alpha + cœur clair) et non en carrés : à deux pixels de côté, un carré se voit. Ce sont des `arc()`, pas des dégradés radiaux — c'est le dégradé par particule qui met un téléphone à genoux, pas le tracé.
