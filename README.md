...
- **Chacun relit ses propres créneaux dans l'encart** : les présents de chaque créneau sont listés **sous la ligne de couverture correspondante**, en italique. C'est le **seul endroit possible** — un menu déroulant Discord appartient au message, pas au lecteur, donc il ne peut pas être pré-coché différemment pour chaque personne. Sans ça on coche et on ne sait plus ensuite ce qu'on a mis.

  > ✅ **08–10** · 7 présents — complet
  > *Abarrach, Lorhelyne, Bahlor, Desta, Merlin, alio, Frya*
  > ⚠ **12–14** · 6 présents — manque transporteur, moissonneur
  > *Bahlor, Neuroch, Quatrequart, Merlin, alio, Frya*

  **Deux versions rejetées avant celle-ci**, et les deux échecs sont instructifs :
  1. *La plage collée à chaque nom dans les colonnes de postes.* Ces colonnes sont `inline`, donc larges d'un tiers d'encart : « Abarrach · 08–10, 14–16 » y tenait sur deux lignes et l'encart devenait un mur en escalier. **On ne peut pas élargir un encart** — la largeur appartient au client Discord. Le seul levier est `inline`, qui décide entre colonne étroite et pleine largeur.
  2. *Un champ pleine largeur groupé par plage* (`08–12 · Abarrach, Desta`). Plus court dans les cas favorables, mais **non borné** : avec quatre blocs il existe **quinze combinaisons**, donc douze inscrits pouvaient produire douze lignes.

  Une entrée **par créneau** est bornée par construction : autant de lignes qu'il y a de blocs, jamais plus, et la structure est la même que la couverture — d'où la fusion des deux champs. **Garde-fou de longueur** : un champ d'encart plafonne à 1024 caractères et un message rejeté l'est *en entier*. Le budget par créneau est donc **adaptatif** (`900 / nb_créneaux − 80`), la liste étant tronquée en « … +N » au-delà. Mesuré à 25 inscrits partout : 823 octets sur 4 créneaux, 789 sur 5, 755 sur 6. ⚠ `strlen()` et **jamais `mb_strlen()`** — pas de mbstring sur ce serveur. Les blocs consécutifs sont **fusionnés en plages** (`creneaux_resume()`) parce que les gens pensent en tranches continues (« je suis là le matin »), pas en blocs de deux heures : `[0,1]` → `08–12`, `[0,1,2,3]` → `08–16`, `[0,3]` → `08–10, 14–16`. Un index hors plage (durée raccourcie depuis) est ignoré silencieusement. Rien d'affiché quand rien n'est déclaré — l'absence vaut « disponible partout », inutile de l'écrire sous chaque nom.
