...
- **Sélecteur en tuiles** (2026-08) : `/sortie creer` **sans argument** répond par un message **éphémère** en **composants V2** (drapeau `1<<15` = 32768) : une **Media Gallery** d'images carrées, une par activité, puis une rangée de boutons. Le clic sur un bouton ouvre directement le formulaire. L'argument `type` reste disponible et **passe outre les tuiles** — voie rapide préservée pour qui connaît la commande. Trois contraintes du format, toutes structurantes : un message V2 ne peut porter **ni `content` ni `embeds`** (tout passe par `components`) ; une galerie accepte **10 images maximum** et **ses images ne sont pas cliquables** (d'où les boutons séparés, l'ordre des boutons étant le seul lien avec les tuiles) ; **40 composants par message, imbriqués compris** (ici 13 pour 9 activités — la galerie est donc la vraie limite). Un élément de galerie n'ayant **aucun libellé visible** (seulement un texte alternatif), **le nom de l'activité est incrusté dans l'image**. Tuiles : `epice/img/activites/<type>.webp`, 512×512, ~40 Ko pièce, URL absolue construite depuis `site_url`.
