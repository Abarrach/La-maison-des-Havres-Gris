...
- ⚠ **`creneauxSortie()` / `creneauCouverture()` sont un MIROIR JS de `discord_sortie.php`**, et c'est signalé dans le code. `data-api.php` ne peut pas inclure `discord_sortie.php` : c'est un endpoint qui **s'exécute au chargement**. Si le découpage change côté Discord — `RALLY_SEUIL_H`, `RALLY_BLOC_H`, `RALLY_MINIMUM`, la formation minimale — il faut le changer **aux deux endroits**, sinon l'encart et la grille disent deux choses différentes sur la même sortie, ce qui est pire que pas de grille du tout. Les deux implémentations ont été passées sur le **même jeu d'inscriptions** : sorties JSON identiques au caractère près.
